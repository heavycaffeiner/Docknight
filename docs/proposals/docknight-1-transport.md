# Realtime Transport Protocol - Spec Proposal (v2)
...
| Reviewers  |                                  |
| Supersedes | docknight-1-transport (2026-08-09) |
...
event namespaces, the closed set of error codes, the client connection lifecycle including mobile
reconnection, and flow control for terminal output.

Changes from v1: the application-level `ping` and `pong` messages and the full mobile reconnection
behaviour, which v1 added after the fact, are part of the base protocol here. `PROTOCOL_VERSION`
remains 1; the wire format is unchanged.

## 2. Background & Motivation

Almost everything Docknight does is either a command that produces streamed output or a state change
that other viewers must see immediately. A request-response HTTP API would need polling for all of
it, so the transport is a persistent bidirectional connection.

Three properties decide the design.

- Most interactions are request and response with a result the caller waits for, so correlation is
  built into the envelope.
- Some interactions are pure server-to-client pushes: terminal bytes, status changes, list
  snapshots. These need no correlation and are not forced into a request shape.
- Docknight can manage stacks on other Docknight hosts, so a request may be forwarded one hop and an
  event arriving from another host must be labelled with where it came from. The host is a
  first-class field in the envelope, never carried ad hoc in payloads.

A fourth property is elevated to a design driver in this revision: **the primary client is often a
phone.** A mobile browser drops the socket on every app switch, throttles timers in the background,
and runs none while the device sleeps. The connection lifecycle in 4.3.6 is specified against that
reality rather than patched for it later.

A general-purpose realtime library would supply reconnection and acknowledgement callbacks, and also
transport negotiation, its own packet encoding, room management, and a large client bundle. A
protocol of three message shapes, declared once as TypeScript types both sides import, is smaller
than the integration would be and makes a method mismatch a type error.

## 3. Goals & Non-Goals

### 3.1 Goals

- [ ] The WebSocket endpoint, its upgrade handling, and its origin check.
- [ ] The three message envelopes, the keepalive probes, and their encoding.
- [ ] Request correlation, timeouts, and cancellation.
- [ ] The `endpoint` field and its three routing modes: local, one host, all hosts.
- [ ] The complete method namespace and the complete event namespace.
- [ ] The error shape and the closed set of error codes.
- [ ] The client connection lifecycle: reconnect, hold-and-send, liveness probing, generation
      counter, all specified for mobile from the start.
- [ ] Flow control and backpressure for high-rate terminal output.
- [ ] Protocol versioning and the compatibility rule between hosts.

### 3.2 Non-Goals

- [ ] The semantics of any individual method; each is specified in its owning proposal.
...
- [ ] A public REST API. Docknight has no HTTP API other than static assets.
- [ ] Binary framing, compression, or a schema-compiled encoding.
...
- `common/protocol.ts`: envelope types, method map, event map, error codes, `PROTOCOL_VERSION`.
  Both sides import it; a method added on one side without the other fails to type check.
- `backend/ws/server.ts`: upgrade, per-connection state, send queue.
- `backend/ws/router.ts`: dispatch: authentication gate, endpoint routing, handler invocation,
  error mapping.
- `frontend/src/lib/connection.svelte.ts`: connect, reconnect, `request()`, event subscription.

### 4.2 Data Model Changes

No change. Connection state lives in memory for the lifetime of the socket.

### 4.3 Core Logic

#### 4.3.1 Endpoint and upgrade

One endpoint: `GET /ws`, upgraded from the HTTP server defined in proposal 0.

Upgrade checks, in order. Any failure responds `400 Bad Request` and destroys the socket:

1. If an `Origin` header is present, its host must equal the request `Host`. A missing `Origin` is
   allowed, because host-to-host links do not send one. This prevents a page on another origin from
   driving Docknight through ambient credentials.
2. The `X-Docknight-Endpoint` header, when present, marks the connection as an inbound management
   link from another Docknight host; its value becomes the connection's `endpoint` label. Absent
   means a browser client.
...
- `id` is assigned by the client, is a positive integer, and is unique among that connection's
  outstanding requests. The server echoes it unchanged. Reusing an id still in flight is answered
  with `error.duplicateRequestId`.
- `endpoint` is `""` for the local host, `"host:port"` for one remote host, or `"*"` for a broadcast
...
- `ping` and `pong` carry nothing and need no authentication. They exist alongside the
  protocol-level keepalive because a browser answers protocol pings inside the WebSocket
  implementation, where page code cannot see them. A client that suspects its socket died while the
  device slept sends `ping` and treats silence as a dead socket.
...
feature depends on it. Operations that must not overlap, such as two compose commands on one stack,
are serialised by the per-stack lock in proposal 3, not by the transport.
...
- Client side: `request()` rejects locally with `error.timeout` after 30 seconds by default. Methods
  that legitimately run long (`stack.deploy`, `stack.update`, `stack.start`, `stack.stop`,
  `stack.restart`, `stack.down`, `stack.delete`) pass `{ timeout: 0 }` and rely on the terminal
  event stream for progress. A fired local timeout sends `{"t":"cancel","id":N}` so the server can
  stop work it no longer needs.
- Forwarding side: `agentPool.request` rejects with `error.agentTimeout` after 60 seconds for
  ordinary methods and never for the long-running set. A dead link rejects immediately with
...
- Pending requests are rejected, never silently retried. A `stack.delete` in flight when the socket
  dropped may or may not have run; retrying it automatically is the wrong default. The UI reports
  the failure and the next `stackList` event shows the true state.
- After a successful re-login the client discards its cached stack list, host list, and settings and
  waits for the server's `stackList` and `agentList` events. Terminal views re-issue
  `terminal.join`, which replays the server-side scrollback defined in proposal 4.

A phone drops the socket every time the user switches apps. The rest of this section makes that
invisible rather than merely survivable, and it is part of the base contract, not an optimisation:

- **Requests are held, not failed.** A request made while the socket is down waits up to 15 seconds
  for a usable one before giving up. Nothing is retried this way, because nothing was sent. A
...
- **Silence is probed, not trusted.** A socket can outlive the network under it, and the
  protocol-level pong a browser answers with is invisible to page code. The client sends
  `{ t: "ping" }` every 25 seconds and drops the socket if nothing at all arrives within 8 seconds
  of a probe. The drop closes with 4000 rather than waiting on a closing handshake the dead peer
  will never finish.
- **Coming back to the front is what reconnects.** `visibilitychange` to visible, `pageshow`,
  `focus`, and `online` each probe an open socket or start a fresh connect, throttled to one per
  second because waking a device fires several at once.
- **Retries do not grow, and none are scheduled while the app is away.** A drop while the page is
  visible retries every 2 seconds; a drop while hidden schedules nothing, because a backgrounded tab
  has throttled timers and a sleeping phone runs none. That removes the case a growing backoff
  exists to handle: an app returning to the front never has a long delay left to wait out.
- **A short drop never reaches the user.** The banner is driven by a `degraded` flag set 2 seconds
  after a drop, not by the socket state, so a reconnect the user would not have noticed does not
  flash a warning.
- **Views rejoin on a generation counter.** `connection.generation` is bumped once each new socket
  has settled. A terminal pane keyed on it re-issues `terminal.join` and skips the `terminal.leave`
  in its teardown, because leaving on a reconnect would read as the last viewer departing and close
  the shell the rejoin is about to ask for.

The client reloads the whole page when a received `info` event reports a `version` different from
the one it started with, because the served bundle no longer matches the server.

#### 4.3.7 Backpressure

Terminal output is the only stream that can outrun a client. Two mechanisms:

- Coalescing. Output chunks for one terminal accumulate and flush on a 16 millisecond timer, so a
  container printing thousands of short lines produces at most 62 frames per second per terminal.
- Bounded queue. Before each send the connection checks `socket.bufferedAmount`. Above 4 MiB,
  further `terminalWrite` events for that connection are dropped and a counter incremented; when the
  ...
  never dropped. Above 16 MiB the connection closes with code 1013.

Dropping terminal bytes is acceptable and truncation is announced; dropping a response is not.
...
join sets. Host-to-host links apply the same rule in both directions.

#### 4.3.9 Versioning

`PROTOCOL_VERSION` is an integer, currently 1, bumped when a method is removed, a method's
parameters change incompatibly, or an event payload changes incompatibly. Adding a method, an
optional parameter, or an event does not bump it.

An outbound management link sends `X-Docknight-Protocol: <n>`. The remote host rejects a mismatch
during upgrade; the manager surfaces it as an `agentStatus` event with
...
/**
 * Send a request and resolve with its result. Waits up to 15 s for a usable socket
 * when disconnected, and for the auth handshake to settle when one is in flight.
 *
 ...
 * throws ValidationError on any mismatch; handlers never see unchecked input.
 */
...
`ProtocolError.code` is a closed set. The UI decides presentation from the code; `message` is for
logs.
...
| 4000 | Client-side liveness probe went unanswered          |

Unexpected exceptions map to `internal`; the original error is logged with a stack trace and the
...
| Phase 6 | Client: connect, hold-and-send, liveness probe, wake handlers, generation counter, `request`, `on` | TBD               | heavycaffeiner |
| Phase 7 | Endpoint routing hooks, stubs until proposal 5 provides the pool                                  | TBD                | heavycaffeiner |
| Phase 8 | Protocol conformance tests: malformed frames, oversize frames, duplicate ids, cancel, timeouts, probe drop | TBD        | heavycaffeiner |
...
|---------|-----------------------|-------------------------------------------------------------------------------------------------|
| `ws`    | Server-side WebSocket | Node has no server WebSocket implementation. The browser `WebSocket` global covers the client     |
...
- Page lifecycle and background throttling: https://developer.chrome.com/docs/web-platform/page-lifecycle-api
