...
const pending = new Map<
    number,
    { resolve: (data: unknown) => void; reject: (err: unknown) => void; timer?: number }
>();
...
export function connect(): void {
    ...
    connection.update((s) => ({ ...s, phase: "connecting" }));
        ...
        if (msg.t === "res") {
            ...
            if (req !== undefined) {
                ...
                if (msg.ok) {
                    ...
                    req.reject(
                        new AppError(msg.error.code, msg.error.message, msg.error.i18n, msg.error.values),
                    );
                }
            }
            return;
...
function handleDisconnect(): void {
    if (connection.get().phase === "disconnected") return;
    connection.update((s) => ({ ...s, phase: "disconnected" }));
    stopPingTimer();
    ...
    if (degradeTimer === undefined) {
        degradeTimer = window.setTimeout(() => {
            connection.update((s) => ({ ...s, degraded: true }));
        }, 2000);
    }
...
export function transportInit(): void {
...
export function setAuthed(authed: boolean): void {
    connection.update((s) => ({ ...s, phase: authed ? "authed" : "connected" }));
}
...
export function request<T = unknown>(
    ...
    const phase = connection.get().phase;
    if (phase === "connected" || phase === "authed") {
        return perform();
    }
