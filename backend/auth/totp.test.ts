...
/**
 * RFC 6238 appendix B, the SHA-1 rows. The published codes are 8 digits; Docknight uses 6, so
 * the expected values are the last six of each.
 */
...
test("checkCode accepts the neighbouring steps", () => {
    ...
    const next = generateCode(Buffer.from("12345678901234567890", "utf8"), step + 1);
    assert.equal(checkCode(secretBase32, next, null, now).step, step + 1);
});

test("checkCode rejects a replay at or below the last accepted step", () => {
    const secretBase32 = base32Encode(Buffer.from("12345678901234567890", "utf8"));
    const now = 59_000;
    const step = currentStep(now);
    const code = generateCode(Buffer.from("12345678901234567890", "utf8"), step);
    ...
    assert.equal(checkCode(secretBase32, code, step + 1, now).step, null);
    assert.equal(checkCode(secretBase32, code, step - 1, now).step, step);
});

test("checkCode rejects anything that is not six digits, before touching crypto", () => {
    const secretBase32 = base32Encode(Buffer.from("12345678901234567890", "utf8"));
    for (const bad of ["", "12345", "1234567", "abcdef", "12 345", "123.45"]) {
        assert.equal(checkCode(secretBase32, bad, null).step, null, bad);
    }
});

test("checkCode returns null rather than throwing on an unparseable secret", () => {
    assert.equal(checkCode("not-valid-base32!!!", "123456", null).step, null);
    assert.equal(checkCode("", "123456", null).step, null);
});
